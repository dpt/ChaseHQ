 ...
 * stages
 *
 * Conv: host-side index table mapping game stage number to its data struct; the
 *       original Z80 code addresses each stage's tables directly via
 *       self-modified pointers rather than a single array, so there is no
