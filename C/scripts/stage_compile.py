...
streams are columns. See C/docs/stage-text-format.md for the full specification.

Usage:
  python3 stage_compile.py compile   <file.map>              > fragment.c
  python3 stage_compile.py decompile <StageNData.c>          > stageN.map
...
OBJ_DEF_RE = re.compile(r"#define\s+MAP_OBJ_(?:S(\d+)_)?(\w+)_VAL\s*\(?(\d+)\)?")
# Object names defined once in Stages.h and shared by every stage (no S<n>_
# prefix on the macro). Keep in sync with the "common across all stages"
# block there.
COMMON_OBJ_NAMES = {"NONE", "TUNNEL_LIGHT", "SHORT_POLE"}
...
def tokenize_array(body: str, array_name: str) -> List[Item]:
    ...
    for match in TOKEN_RE.finditer(body):
        ...
        elif macro.startswith("MAP_OBJ_"):
            name = re.sub(r"^MAP_OBJ_(?:S\d+_)?", "", macro)
    ...
    def _read_objects(self):
        ...
        for source in sources:
            for stage, name, value in OBJ_DEF_RE.findall(source):
                if stage and int(stage) != self.stage:
                    continue
...
def decompile(path: str, allow_desync: bool, warn) -> str:
    ...
    lines = [
        "; Decompiled from %s by stage_compile.py." % os.path.basename(path),
        "; One row is one slice of road, read bottom to top: the last row is",
        "; the start of the stage. See C/docs/stage-text-format.md.",
...
def mapping_obj_value(mapping: MapText, name: str) -> int:
    ...
    for candidate in (
        ...
        for stage, obj_name, value in OBJ_DEF_RE.findall(open(candidate).read()):
            if (not stage or int(stage) == mapping.stage) and obj_name == name:
                _OBJ_VALUE_CACHE[key] = int(value)
                return int(value)
    raise MapError(
        "no MAP_OBJ_%s_VAL is defined for stage %d in Stage%dData.c or Stages.h"
        % (name, mapping.stage, mapping.stage)
    )
...
def encode_objects(section: Section, stream: str, stage: int, names: Dict[int, str]) -> List[str]:
    ...
    for value, count in runs_of(getattr(section, stream)):
        name = "NONE" if value == 0 else names.get(value, "OBJ%d" % value)
        if name in COMMON_OBJ_NAMES:
            macro = "MAP_OBJ_%s" % name
        else:
            macro = "MAP_OBJ_S%d_%s" % (stage, name)
        for chunk in split_run(count, NIBBLE_MAX):
...
def compile_text(text: str, path: str) -> str:
    ...
    out = [
        "/* Generated by C/scripts/stage_compile.py from %s."
...
def _stage_obj_names(stage: int) -> Dict[int, str]:
    ...
    for candidate in (
        ...
        for found, name, value in OBJ_DEF_RE.findall(open(candidate).read()):
            if not found or int(found) == stage:
                names.setdefault(int(value), name)
    return names
