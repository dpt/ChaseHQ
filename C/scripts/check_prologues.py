...
def check_function(filename, lineno, comment_lines, return_type, params, func_name):
    ...
    for p in split_paragraphs(content):
        if len(CONV_NOTE_RE.findall(" ".join(p))) > 1:
            errors.append(
                "multiple 'Conv:' notes in one paragraph, split into separate paragraphs: %r"
                % p
            )
    ...
    if content[0].startswith("$"):
        if not TITLE_RE.match(content[0]):
            errors.append(
                "title line missing '$XXXX: description' format: %r" % content[0]
            )
    ...
    if missing_names:
        errors.append("missing \\param for parameter(s): %s" % ", ".join(missing_names))
    ...
    if content[idx:]:
        tail_lines = [
            l
            for p in split_paragraphs(content[idx:])
            if p[0].startswith("Conv:")
            for l in p
        ]
...
def check_file(filename):
    results = []
    for lineno, func_name, comment_lines, return_type, params in extract_functions(
        filename
    ):
        errors = check_function(
            filename, lineno, comment_lines, return_type, params, func_name
        )
...
def check_order(filename):
    ...
    for lineno, func_name, comment_lines, _return_type, _params in extract_functions(
        filename
    ):
