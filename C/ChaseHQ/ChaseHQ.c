...
static void forked_road_plotter(chqstate_t *state, u8 *IX, u8 *IY);
...
continue_after_right_hand_done:
    IXtable_ea00++;
    WRAPPING(HLroadbuf, 32, state->road_buffer_start); // advance to left-side column
    assert(HLroadbuf >= state->road_buffer_start && HLroadbuf < state->road_buffer_end);
                          ...
                          const u8         *IY)
{
  ...
  if (IX[1] == 0) // buffer offset/distance
    draw_stretchy_object_left(state, Bparam, DEstretchy, IX, IY);
  // POP BC/HL/IX
  ...
  Cdepth = HLpair->depth;
  HL = (const u8 *) &DEpairs[10] + Bminheight * 3; // depthsets seem to have more data than expected...
                                        ...
                                        const u8         *IY)
{
  ...
  for (;;) {
    ...
    SM_91CD_callback(state, Bpairdepth, HLbitmap, IX, IY);
...
dso_continue:
  ...
  SM_9244_callback(state, Bpairdepth, HLbitmap, IX, IY); // does this update B?
...
dh_adfa:
  ...
  if (An_hazards) {
    ...
    do {
      HLtable++;
      if (Ddistance >= HLtable[-1]) { // these offsets are bound to be wrong due to byte/word
                                  ...
                                  const u8   *IY)
{
  ...
  do {
    ...
    IXhazard = &state->hazards[*HLtable++]; // sampled = $A19C/hazard_1 $A1B0/hazard_2
...
static void exit_fork(chqstate_t *state)
{
  ...
  if (state->fork_taken == 1) {
    ...
    state->rm_SM_BE6D_curvature_one_command_ptr = lookup_map_goto(state, state->rm_SM_BBC2_rightfork_curve);
    state->rm_SM_BEBF_height_one_command_ptr    = lookup_map_goto(state, state->rm_SM_BBC5_rightfork_height);
    state->rm_SM_BF0A_lanes_one_command_ptr     = lookup_map_goto(state, state->rm_SM_BBC8_rightfork_lanes);
    state->rm_SM_C046_hazards_one_command_ptr   = lookup_map_goto(state, state->rm_SM_BBB8_rightfork_hazards);
    state->rm_SM_BF84_rightside_one_command_ptr = lookup_map_goto(state, state->rm_SM_BBBB_rightfork_rightside);
    state->rm_SM_BFCD_leftside_one_command_ptr  = lookup_map_goto(state, state->rm_SM_BBBE_rightfork_leftside);
    obj_offset = 32; /* rightobjs: ROADBUF_LANES_OFFSET + 32 = ROADBUF_RIGHTOBJS_OFFSET */
  } else {
    /* $BB74: ef_left */
    curve_type = 0xFC;
    lanes_type = 0x01;
    ...
    state->rm_SM_BE6D_curvature_one_command_ptr = lookup_map_goto(state, state->rm_SM_BB95_leftfork_curve);
    state->rm_SM_BEBF_height_one_command_ptr    = lookup_map_goto(state, state->rm_SM_BB98_leftfork_height);
    state->rm_SM_BF0A_lanes_one_command_ptr     = lookup_map_goto(state, state->rm_SM_BB9B_leftfork_lanes);
    state->rm_SM_C046_hazards_one_command_ptr   = lookup_map_goto(state, state->rm_SM_BB8B_leftfork_hazards);
    state->rm_SM_BF84_rightside_one_command_ptr = lookup_map_goto(state, state->rm_SM_BB8E_leftfork_rightside);
    state->rm_SM_BFCD_leftside_one_command_ptr  = lookup_map_goto(state, state->rm_SM_BB91_leftfork_leftside);
    obj_offset = 64; /* leftobjs: ROADBUF_LANES_OFFSET + 64 = ROADBUF_LEFTOBJS_OFFSET */
  }
...
static void draw_road_scene_change(chqstate_t *state, u8 *IX, u8 *IY)
{
  u8   dist;         /* IYl = byte offset of IY within table_e300 */
  u8   H;            /* left-hand table hi byte ($E8/$E9/$EA/$EB/$EC) */
  u8   lane_flags;   /* IX[0] */
  u8   L_sp;         /* L = ~((0x60 - IY[0]) << 1) -- always odd */
  u8   range;        /* Bresenham step count = IY[0] - C_val */
  ...
  /* $C2E7: load H = left-hand table hi byte from IX[0] bits 0-1 */
  H    = (u8)((IX[0] & 3) + 0xE7);
  dist = (u8)(IY - &state->table_e300[0]);
  if (dist >= 19)
    goto c43b;
  ...
  /* $C2FA-$C304: adjust H based on bits 5 and 7 of IX[0] */
  ...
  C_val  = (lane_flags & 0x0C) == 4 ? IY[2] : IY[1];
  range  = (u8)(IY[0] - C_val);
  if (range == 0 || (range & 0x80))
    goto c439;
  L_sp    = (u8)(~((u8)(0x60 - IY[0]) << 1));
...
static void draw_road(chqstate_t *state)
{
  ...
  u8  Chorizon;        /* was C */
  u8 *IXplanes;        /* was IX */
  u8  Blanedataoffset; /* was B */
  int carry_stripe;    /* was carry */
  u8  Hopcode;         /* was H */
  u8  Lstripe_height;  /* was L */
  u8  Aopcode;         /* was A */
  ...
  state->dr_SM_C6B2_initial_stripe_state = Blanedataoffset & 1;
  carry_stripe = !!(Blanedataoffset & (1 << 1)); // test bit 1 + ensure boolity

  Hopcode = 0xD0; // opcode of RET NC
  Lstripe_height = 16;
  Aopcode = Hopcode;
  Bfill_pattern = _X_X_X_X; // chequerboard fill pattern
  if (!carry_stripe) {
    Hopcode = 0x00; // opcode of NOP
    Lstripe_height = 48;
    Bfill_pattern = ________; // empty fill pattern
  }

  state->dr_SM_C6D3_ret_nc_or_nop = Aopcode; // always $D0 (RET NC)
  state->dr_SM_C677_ret_nc_or_nop = Hopcode; // $D0 (RET NC) or $00 (NOP)
  ...
  state->dr_SM_C6BC_fill_pattern = Bfill_pattern;

  // passing L to HL arg here - not clear just now if all of HL required or just L
  dr_read_lanes(state, IXplanes, IYpheight, Bfill_pattern, Chorizon, DEbackbuf, Llane_mask); // was FALLTHROUGH
}
 ...
 * \param[in] L             TBD
 */
static void dr_read_lanes(chqstate_t *state, u8 *IXplanes, u8 *IYpheight, u8 Bfill_pattern, u8 Chorizon, u16 DEbackbuf, u8 L)
{
  int carry = 0;
  u8  Aleft_offset;         /* was A */
  u8  Ldash_lanes;          /* was L' */
  u8  Aleft_hand_table_hi;  /* was A */
  u8  Cdash_C5AC;           /* was C' */
  u8  Hdash_in_tunnel_flag; /* was H' */
  u8  A_tunnel_visible;     /* was A */
  s8  Cdash;
  s8  Acopy_of_cdash;
  s8  Bcopy_of_cdash;
  ...
  if (Aleft_offset == 0) {
    ...
    state->dr_SM_C4B2_callback(state, Bfill_pattern, DEbackbuf, L); // exit via
    return;
  }
  ...
  state->dr_SM_C642_left_hand_table_hi = Aleft_hand_table_hi;
  state->dr_SM_C5B3_left_hand_table_hi = Aleft_hand_table_hi;
  ...
  if ((Ldash_lanes & (1 << 7)) == 0) {
    ...
    state->dr_SM_C5D9_right_hand_table_hi = Aleft_hand_table_hi;
    state->dr_SM_C68A_right_hand_table_hi = Aleft_hand_table_hi;

    state->dr_SM_C5AC_neg_lane_count = Cdash_C5AC; // Conv: A removed
  ...
  if (carry == 0) {
    ...
    if ((Ldash_lanes & (3 << 3)) != 0) {
      // Tunnel transition
      state->dt_SM_C15D_tunnel_distance = IYpheight - &state->table_e300[0];
      A_tunnel_visible = 1;
      Cdash = 0; // was INC C
      ...
      state->dt_SM_C160_tunnel_visible = A_tunnel_visible;
    }

    state->dr_SM_C88F_in_tunnel = Hdash_in_tunnel_flag;
    state->dr_SM_C5D9_right_hand_table_hi = 0xEB;
    state->dr_SM_C68A_right_hand_table_hi = 0xEB;
    state->dr_SM_C5AC_neg_lane_count = -1;
...
static void dr_four_lane_highway(chqstate_t *state, u8 Bfill_pattern, u16 DEbackbuf, u8 L)
{
  ...
  state->dr_SM_C642_left_hand_table_hi  = 0xE8;
  state->dr_SM_C5B3_left_hand_table_hi  = 0xE8;
  state->dr_SM_C68A_right_hand_table_hi = 0xEC;
  state->dr_SM_C5D9_right_hand_table_hi = 0xEC;

  state->dr_SM_C5AC_neg_lane_count = -4;

  dr_c54d(state, Bfill_pattern, DEbackbuf, L, dr_c551); // was FALLTHROUGH
}
 ...
 */
static void dr_c54d(chqstate_t *state, u8 Bfill_pattern, u16 DEbackbuf, u8 L, dr_callback_t *HLdash_callback)
{
  state->dr_SM_C4B2_callback = HLdash_callback;
  // EXX - UNBANK (we enter banked)
  dr_c551(state, Bfill_pattern, DEbackbuf, L); // was FALLTHROUGH
}
...
static void dr_c565_unfilled_path(chqstate_t *state, u16 DEbackbuf)
{
  ...
  Cdash = Ldash;
#if 0
  dr_c62e(state); // exit via   (jump to jump table; it's a label!)
#endif
}
...
dr_c62e: // NEED TO SPLIT HERE
  ...
  // EXX
  H = state->dr_SM_C642_left_hand_table_hi; // load ($E8/$E9/$EA for left/centre-left/centre)
  A = *HL;
  if (A)
    goto dr_c667_loop;
...
dr_c667_loop:
  for (;;) {
    ...
    // EXX
    E = A;
    L = ((A & 7) << 1) + state->dr_SM_C677_ret_nc_or_nop;
    E = ((E >> 3) & 31) + B;
    *DEbackbuf++ = *HL++, BC--;
...
dr_c68a:
  H = state->dr_SM_C68A_right_hand_table_hi; // eg. $EC
  A = *HL;
  L--;
  if (A)
    goto dr_c6ab;
...
dr_set_stripes:
  A = state->dr_SM_C6B2_initial_stripe_state ^ 1; // loads (<lanes data offset> & 1) and toggles
  state->dr_SM_C6B2_initial_stripe_state = A;
  if (A)
...
dr_stripe_perhaps_off:
  ...
  state->dr_SM_C677_ret_nc_or_nop ^= state->dr_SM_C6D3_ret_nc_or_nop;
...
dr_stripe_perhaps_on:
  ...
  A = state->dr_SM_C6D3_ret_nc_or_nop + 16;
  if (carry)
    goto dr_edge_width_handled;

  state->dr_SM_C6D3_ret_nc_or_nop = A;
  C = A;
  A = state->dr_SM_C677_ret_nc_or_nop;
  if (A == 0)
    goto dr_c6f6;
  A = C;
  state->dr_SM_C677_ret_nc_or_nop = A;
...
static void forked_road_plotter(chqstate_t *state, u8 *IX, u8 *IY)
{
  ...
  sm_CB5D  = state->dr_SM_C6D3_ret_nc_or_nop;
  sm_CA9D  = state->dr_SM_C677_ret_nc_or_nop;
  sm_CB00  = state->dr_SM_C677_ret_nc_or_nop;
...
frp_cb90: /* $CB90 */
  {
    u8 old_h = *IY;
    IY++;
    WRAPPINGINCREMENT(IX, state->road_buffer_start);
    A = (u8)(old_h - *IY);  /* $CB97: SUB (IY+$00) */
...
static void build_curve_table(chqstate_t *state, int forked)
{
  ...
  assert(A >= 0 && A <= 95);
  IX = &curvature_to_xpos[A]; // table is 16-bit
  ...
  do {
    ...
    int IXl = (IX - &curvature_to_xpos[0]) * 2 + 0x40;
    IXl = (IXl + curvature_A) & 0xFF;
    if (IXl < 0x40) IXl = 0x40;
    IX = &curvature_to_xpos[(IXl - 0x40) / 2];
