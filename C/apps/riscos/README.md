...
RISC_OSLib and the Wimp application shell, starts a silent 128K game directly
in single-tasking fullscreen mode, and exits back to the saved desktop when
Escape is pressed:
...
- `Makefile,fe1` — AMU makefile (root-relative paths; run from the repository root)
- `Riscos.c`, `Host.c`/`Host.h`, `Fullscreen.c`, `Tests.c` — native host sources
- `Resources/` — `!ChaseHQ` application resources (`!Boot`, `!Run`, sprites, templates)
