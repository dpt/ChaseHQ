...
- `dust_stones_stuff` (`$AA33–$AA34`, `ADD A,E; RET NC`): `if (A >= E) return` made the draw call unreachable. Fix: `A += E; if (A <= 255) return;`.
...
**Rule:** For subtraction, use a direct unsigned comparison (`if (L < E)`) — never `(s8)` or `& 0x80` to recover carry. For addition, add first and check the sum against 255 (`if (A > 255)` / `if (A <= 255)`), never the inputs or the sign bit. When the accumulator is `int`, compare `sum > 255`, not the unsigned-wrap idiom. When two conditional jumps follow one `SUB` (`JR Z` then `JR NC`), read them together — the combined condition is usually `<= 0`.
