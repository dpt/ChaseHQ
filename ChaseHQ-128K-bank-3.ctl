...
C $C660,3 fresh #R$8002@main (score_bcd) of $87654321 (a
...
C $C7F0,3 from #R$D272@bank1 (indexed by the countdown value),
C $C7F3,3 negate it and add to Y velocity, then advance the
C $C7F6,2 countdown
@ $C804 label=oss_lookup_speed
C $C804,1 Helper: speed = #R$D272@bank1[(B<<8 | C)] >> 2 -- decodes
...
D $CC50 Four position-encoded text strings (2 position bytes, 1 attribute byte, ASCII text terminated by a character with bit 7 set -- same encoding as the high-score text at #R$C3AF): "(C) 1989 OCEAN SOFTWARE", "(C) 1988 TAITO CORPORATION" (Chase H.Q. was originally a Taito arcade game, licensed to Ocean for the home computer ports), "PRESS GEAR TO PLAY" and "PRESS ENTER FOR OPTIONS" (drawn by #R$C608/#R$C615 via $FDA4). Text ends at $CCB6.
...
B $FA72,295,8*36,7
c $FB99 Options menu driver (reached from the title screen's "PRESS ENTER FOR OPTIONS" prompt, see the text block at #R$CC50). Boots interrupts and tune 0, then loops: draws the control-select screen text (#R$FC29 via $FD9C -- "ENTER OPTION" / P1. SINCLAIR JOYSTICK / P2. CURSOR JOYSTICK / P3. KEMPSTON JOYSTICK / P4. KEYBOARD / P5. DEFINE KEYS) and polls the keyboard (port $FE, keys 1-5 read as a 5-bit mask) to dispatch into one of the five listed choices.
...
D $FC29 Text for the options menu and hidden test/service menu screens (same position+attribute+ASCII-with-bit7-terminator encoding as #R$CC50), printed via $FD9C from pointers set up in #R$FB99/#R$FEA9: - Control select: "ENTER OPTION" / P1. SINCLAIR JOYSTICK / P2. CURSOR JOYSTICK / P3. KEMPSTON JOYSTICK / P4. KEYBOARD / P5. DEFINE KEYS - Key redefinition: "REDEFINE KEYS" / GEAR / ACCELERATE / BRAKE / LEFT / RIGHT / QUIT / PAUSE / TURBO - Test mode: "CHASE H.Q. TEST MODE" / TITLE SCREEN / 1 TO 5. LOGO ANIMATION / 6. SCORE ENTRY / PIN GAME / P1. RESTART LEVEL / P2. NEXT LEVEL / P3. END SCREEN / P4. EXTRA CREDIT
